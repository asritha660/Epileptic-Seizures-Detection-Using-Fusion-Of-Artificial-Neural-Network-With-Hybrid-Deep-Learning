...
# Train the model and plot performance analysis graphs
history = model.fit(
    x=X_train, y=y_train,
    validation_data=(X_val, y_val),
    epochs=20,
...
report = classification_report(np.concatenate(list_true) > 0, np.concatenate(list_pred) > 0.5)
print(report)

roc = roc_curve(np.concatenate(list_true) > 0, np.concatenate(list_pred))
auc = roc_auc_score(np.concatenate(list_true) > 0, np.concatenate(list_pred))

# Plot ROC curve
plt.figure(figsize=(6, 6))
plt.plot(roc[0], roc[1], label=f'AUC = {auc:.2f}')
...
# Analyze performance results and justify the effectiveness of the model in the manuscript
